...
// Read genre data
$actor_data_file = file("director_data", FILE_SKIP_EMPTY_LINES);

foreach ($actor_data_file as $idx => $val_str) {
	$pair = explode('|', $val_str);
	$director_name = trim($pair[1]);
	$imdb_id = trim($pair[0]);

	// Check if the actor exists
	echo "Checking $director_name\n";
	if ($person_fetch_stmt = $link->prepare("SELECT Person_ID FROM FM_Person WHERE Person_Name = ?")) {

		$person_fetch_stmt->bind_param("s", $director_name);
	...
	// Then insert the relationship into FM_Directed
	$insert_result = $link->query("INSERT INTO FM_Directed (`Person_ID`, `IMDB_ID`) values ('$person_id', '$imdb_id')");
}
